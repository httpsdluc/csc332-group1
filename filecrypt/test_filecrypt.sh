#!/bin/bash
#
# Reformatted test suite for filecrypt
# Matches specific output requirements and fixes signal handling race conditions.
#
...
test_signal_handling() {
    ...
    # FIX 1: Increase to 500MB to ensure process runs long enough on fast machines
    # This prevents the process from finishing before we can kill it
    ...
    # FIX 2: Check if process is alive, then Kill. 
    # The '|| true' is CRITICAL: it prevents the script from crashing if the process 
    # dies between the check and the kill.
    if kill -0 "$PID" 2>/dev/null; then
        kill -INT "$PID" 2>/dev/null || true
    fi
    
    # Wait for process and capture status
    # FIX 4: Initialize STATUS and use '|| STATUS=$?' so 'set -e' doesn't crash the script when wait returns 130
    local STATUS=0
    ...
    if [ $STATUS -eq 130 ]; then
        ...
        # FIX 3: Gracefully handle "Too Fast" instead of failing
