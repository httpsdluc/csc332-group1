 ...
 *
 * Author: Diana Lucero
 * Course: CSC 332 - Operating Systems
 * Professor: Zaid T Al-Mashhadani
 * Date: November 2024
 *
 * DESCRIPTION:
 * Monitors and enforces resource limits on executing programs.
 * Terminates processes that exceed time or memory limits.
 *
 * FEATURES:
 * - Time limit enforcement (-t)
 * - Memory limit monitoring (-m)
 * - CPU usage monitoring (-c)
 * - Verbose monitoring mode (-v)
 *
 * USAGE:
 * ./timedexec -t SECONDS -m MEGABYTES COMMAND [ARGS...]
 *
 * EXAMPLE:
 * ./timedexec -t 5 -m 100 ./myprogram
 * (Run myprogram, kill after 5 seconds or 100MB)
 */
